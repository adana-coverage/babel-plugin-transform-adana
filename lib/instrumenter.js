...
export default function ({ Plugin, types, parse, traverse }) {
	 ...
	 */
	function ignore(node, file) {
		return isPragma(node) || node.__adana;
	}
	...
	function pair(options) {
		...
		if (types.isBlockStatement(node)) {
			...
			return assign(node, {
				body: [
					...(pragmas || []),
					assign(types.expressionStatement(marker), { __adana: true }),
					...(statements || [])
				]
			});
	...
	function Statement(node, parent, scope, file) {
		if (!file) { throw new Error('wtf'); }
		if (!ignore(node, file)) {
			return [
				assign(types.expressionStatement(
